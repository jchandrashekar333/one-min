        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
              {gameState === 'reveal' && (
                <div className="flex flex-wrap w-full gap-3 items-center mt-4">
                  <button onClick={() => { playSound('click'); setSpinCount(s => s + 1); }} className="flex-1 md:flex-initial px-6 py-[1rem] bg-zinc-900 text-white rounded-full font-bold text-base md:text-lg tracking-wide hover:bg-zinc-800 transition-all shadow-md active:scale-95 flex items-center justify-center gap-2.5 min-w-[120px]"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8" /><path d="M3 3v5h5" /></svg>Refresh</button>
                  <button onClick={() => { playSound('click'); startSpeaking(); }} className="flex-[2] md:flex-1 py-[1rem] bg-transparent border-[2.5px] border-zinc-900 text-zinc-900 rounded-full font-bold text-base md:text-lg tracking-wide hover:bg-zinc-900 hover:text-white transition-all shadow-sm active:scale-95 flex items-center justify-center gap-2">Start Timer &rarr;</button>
                  <button onClick={copyChallenge} className="flex-1 md:flex-initial px-5 py-[1rem] bg-white border-[2.5px] border-zinc-900 text-zinc-900 rounded-full font-bold text-base md:text-lg hover:bg-zinc-100 transition-all shadow-sm active:scale-95 flex items-center justify-center gap-2 relative min-w-[140px]">
                    {showCopied ? <span className="text-green-600 font-black">Link Copied!</span> : <><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>Challenge</>}
                  </button>
                </div>
              )}
